...
- Melhorada extração do número do processo na visualização do processo: tenta múltiplos scripts e adiciona seletores de fallback para o SEI 4.x.
- Adicionado suporte a seletores alternativos na lista de processos para compatibilidade com SEI 4.x.
- Adicionada robustez na detecção da barra de comandos (`#divComandos`) com múltiplos seletores de fallback.
- Adicionada detecção de linha da tabela que percorre os nós pai para lidar com variações de aninhamento do HTML.
- Adicionado padrão de URL `acao=arvore_visualizar` ao manifest para injetar o script na árvore de processo do SEI 4.x.
